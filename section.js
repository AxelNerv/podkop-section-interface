...
function createSectionContent(section) {
  ...
  o = section.option(
    ...
    _("Интерфейс секции"),
    _(
      "Выберите сетевой интерфейс (точку доступа), на который будет действовать эта секция. Оставьте пустым для отключения.",
    ),
  );
  o.rmempty = true;
  o.depends("connection_type", "proxy");
  ...
  o = section.option(
    form.ListValue,
    "section_interface_mode",
    _("Режим интерфейса секции"),
    _(
      "«Только списки секции» — через секцию идёт только трафик к доменам/подсетям из списков этой секции (например, только YouTube), остальное напрямую. " +
        "«Весь трафик подсети» — весь трафик с интерфейса идёт через секцию, списки при этом могут быть пустыми.",
    ),
  );
  o.value("lists", _("Только списки секции (рекомендуется)"));
  o.value("full", _("Весь трафик подсети"));
  o.default = "lists";
  o.rmempty = true;
  o.depends("connection_type", "proxy");
  o.depends("connection_type", "vpn");
